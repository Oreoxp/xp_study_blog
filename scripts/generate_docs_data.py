...
def extract_doc_info(doc_file):
    ...
    doc_info = {
        ...
        "url": f"/docs/{doc_file.relative_to(Path('_docs')).parent}/{doc_file.stem}/"
    }
...
def generate_jekyll_data(docs_data, output_dir="_data"):
    ...
    with open(yaml_file, 'w', encoding='utf-8') as f:
        yaml.dump(docs_data, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
...
def generate_dynamic_index(docs_data, output_file="_includes/dynamic_docs.html"):
    ...
    for category in docs_data["categories"]:
        ...
        html_content.append(f'        <span class="docs-count">{category["doc_count"]} 个文档</span>')
        html_content.append('    </div>')
        html_content.append('    <div class="docs-list">')
        
        for doc in category["docs"]:
            html_content.append(f'        <a href="{doc["url"]}" class="doc-item">')
            html_content.append(f'            <span class="doc-title">{doc["title"]}</span>')
            if doc["description"]:
                html_content.append(f'            <span class="doc-description">{doc["description"]}</span>')
            else:
                html_content.append(f'            <span class="doc-description">{doc["title"]}相关文档</span>')
            html_content.append('        </a>')
        
        html_content.append('    </div>')
        html_content.append('    <div class="docs-category-footer">')
        html_content.append(f'        <a href="/docs/{category["path"]}/" class="btn btn-outline">查看全部</a>')
        html_content.append('    </div>')
        html_content.append('</div>')
        html_content.append("")
